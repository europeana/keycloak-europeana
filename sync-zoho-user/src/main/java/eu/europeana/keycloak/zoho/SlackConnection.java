...
/**
 * The class encapsulates necessary operations for sending over the
 * Slack message to configured slack webhook
 */
public class SlackConnection {
  ...
  public void publishStatusReport(String message) {
    ...
    try {
      ...
      sendPostRequest(message, slackWebhookApiAutomation);
    } catch (IOException e) {
      LOG.error("Exception occurred while sending slack message !! " + e.getMessage());
    }
  }

  private static void sendPostRequest(String message, String slackWebhookApiAutomation)
      throws IOException {
